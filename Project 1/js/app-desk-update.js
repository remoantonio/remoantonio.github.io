...
$(() => {
    ...
    for (const item of newArr) {
        $('<div>').attr('id', item.pA).addClass('attribute').appendTo($('#container')).css('background-color', item.color).attr('droppable','true').on('dragover', false).on('drop', dropHome)
    }
    ...
    for (let i = 0; i < heroesArr.length; i++) {
        ...
        // let attrPicker = newHero[heroesArr[i].primary_attr]
        let attrPicker = $('#' + heroesArr[i].primary_attr)
        $('<div>').attr('id', i).addClass(heroesArr[i].primary_attr).addClass(heroesArr[i].localized_name).appendTo(attrPicker).css('background-image', 'url(' + img + ')').attr('draggable', 'true').on('mousedown', pickUp).attr('heroID', heroesArr[i].id)
